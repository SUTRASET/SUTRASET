...
% the [2,3,1] is obtained from the orintation of the x y and z.
xnod_mtx=permute(reshape(nod(1).terms{x_idx},[inp.nn1,inp.nn2,inp.nn3]),[2,3,1]);
ynod_mtx=permute(reshape(nod(1).terms{y_idx},[inp.nn1,inp.nn2,inp.nn3]),[2,3,1]);
znod_mtx=permute(reshape(nod(1).terms{y_idx},[inp.nn1,inp.nn2,inp.nn3]),[2,3,1]);
...
a.sub2=subplot('position'...
     ,[fig_pos.left+0.5,fig_pos.bottom-fig_pos.height,...
      fig_pos.length,fig_pos.height]);
a.plot2=plot(time_day(1:nt),...
       arrayfun(@(y) y.terms{sm_idx}(inp.nn1),nod(1:nt)),...
         'r-','linewidth',a.lw);hold on
get(gca,'xtick');
set(gca,'fontsize',12);
xlabel('Time (day)','FontSize',a.fs);
