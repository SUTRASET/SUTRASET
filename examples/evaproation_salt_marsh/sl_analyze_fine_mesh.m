...
x_nod_mtx=reshape(nod(1).terms{x_nod_idx},[inp.nn1,inp.nn2]);
y_nod_mtx=reshape(nod(1).terms{y_nod_idx},[inp.nn1,inp.nn2]);

x_ele_mtx=reshape(ele(1).terms{x_ele_idx},[inp.nn1-1,inp.nn2-1]);
y_ele_mtx=reshape(ele(1).terms{y_ele_idx},[inp.nn1-1,inp.nn2-1]);

et_x_ay=nod(1).terms{x_nod_idx}(bcof(1).i);
et_y_ay=nod(1).terms{y_nod_idx}(bcof(1).i);
...
for n=400:1:length(nod)-1
  ...
  quiver ( x_ele_mtx(1:a.iv:end,1:a.ivy:end),y_ele_mtx(1:a.iv:end,1:a.ivy:end)...
